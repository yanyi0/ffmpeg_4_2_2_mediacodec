...
static int flv_write_header(AVFormatContext *s)
{
    ...
    for (i = 0; i < s->nb_streams; i++) {
        FLVContext *flv = s->priv_data;
        AVCodecParameters* par = s->streams[i]->codecpar;
        if ((flv->flags & FLV_SKIP_WRITE_AVC_HDR) && ( par->codec_id == AV_CODEC_ID_H264 || par->codec_id == AV_CODEC_ID_MPEG4)) {
            av_log(s, AV_LOG_INFO, "Skip AVC sequence header when write header\n");
            if (par->extradata) {
                //insure flv_write_packet will write
                memset(par->extradata, 1, par->extradata_size);
            }
            continue;
        }
        flv_write_codec_header(s, par, 0);
    }
...
static const AVOption options[] = {
    ...
    { "skip_write_avc_header", "Skip AVC sequence header when write header", 0, AV_OPT_TYPE_CONST, {.i64 = FLV_SKIP_WRITE_AVC_HDR}, INT_MIN, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM, "flvflags" },
    { NULL },
};
