                                            ...
                                            struct MvField mergecandlist[])
{
    ...
    is_available_b0 = AVAILABLE(cand_up_right, bB0) &&
                      xbB0 < s->ps.sps->width &&
                      PRED_BLOCK_AVAILABLE(bB0) &&
                      !is_diff_mer(s, xbB0, ybB0, x0, y0);

    if (is_available_b0 &&
        !(is_available_b1 && COMPARE_MV_REFIDX(bB0, B1))) {
        mergecandlist[nb_merge_cand] = TAB_MVF_PU(bB0);
        if (merge_idx == nb_merge_cand)
            return;
        nb_merge_cand++;
    }
...
b_candidates:
    ...
    // above right spatial merge candidate
    if (is_available_b0) {
        if (MP_MX(bB0, pred_flag_index_l0, mxB)) {
            goto scalef;
        }
        if (MP_MX(bB0, pred_flag_index_l1, mxB)) {
            goto scalef;
        }
    }
    ...
    availableFlagLXbB0 = 0;

scalef:
    if (!isScaledFlag_L0) {
        if (availableFlagLXbB0) {
            availableFlagLXA0 = 1;
            mxA = mxB;
        }
        availableFlagLXbB0 = 0;
        ...
        // XB0 and L1
        if (is_available_b0) {
            availableFlagLXbB0 = MP_MX_LT(bB0, pred_flag_index_l0, mxB);
            if (!availableFlagLXbB0)
                availableFlagLXbB0 = MP_MX_LT(bB0, pred_flag_index_l1, mxB);
        }

        if (is_available_b1 && !availableFlagLXbB0) {
            availableFlagLXbB0 = MP_MX_LT(B1, pred_flag_index_l0, mxB);
            if (!availableFlagLXbB0)
                availableFlagLXbB0 = MP_MX_LT(B1, pred_flag_index_l1, mxB);
        }

        if (is_available_b2 && !availableFlagLXbB0) {
            availableFlagLXbB0 = MP_MX_LT(B2, pred_flag_index_l0, mxB);
            if (!availableFlagLXbB0)
                availableFlagLXbB0 = MP_MX_LT(B2, pred_flag_index_l1, mxB);
        }
    }
    ...
    if (availableFlagLXbB0 && (!availableFlagLXA0 || mxA.x != mxB.x || mxA.y != mxB.y))
